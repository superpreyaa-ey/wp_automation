...
def wp_createaudit(request):
    print("inside Workpaper Automation function==========1=========")
    feature = None
    if request.method == 'POST':
        print("inside Workpaper Automation function=========2==========")
        audit_name = request.POST.get('audit_name')
        audit_year = request.POST.get('audit_year')
        flag = request.POST.get('meeting_type')
        feature = request.POST.get('feature', None)
        files = request.FILES.getlist('file')
        
        print(f"Meeting Type:{flag}")
        ...
        try:
            audit, created = Audit.objects.get_or_create(
                ...
                defaults={
                    ...
                    'audit_status': 'Workapaper Report Generated',
            ...
            if not created:
                ...
                audit.audit_status = 'Workapaper Report Generated' #'Audit Updated'  # Assuming you want to change the status on update
        ...
        # =====================================technically it should have audio to text conversion code================================

        script_folder_path = os.path.join('static', 'media', 'project_files', 'audio_script_files')

        # Collect all Word files from the script_folder_path
        word_files = []
        for file_name in os.listdir(script_folder_path):
            if file_name.endswith(('.docx', '.doc')):
                word_files.append(os.path.join(script_folder_path, file_name))

        all_documents = documents_obj + word_files  # Combine audio and Word files

        # Initialize paginator with combined list of documents and Word files
        page = request.GET.get('page', 1)
        paginator = Paginator(all_documents, 1)  # Show 1 document per page
        ...
        # Determine the type of current document (audio or Word file)
        file_type = None
        file_path = None
        data_list = None

        if isinstance(current_document.object_list[0], str):  # It's a Word file path
            file_type = 'Word'
            file_path = current_document.object_list[0]
            absolute_file_path = os.path.abspath(file_path)

            # Extract data from Word file if needed
            try:
                doc = Document(absolute_file_path)
                data_list = [{'text': paragraph.text} for paragraph in doc.paragraphs if paragraph.text]
            except Exception as e:
                data_list = None
                print(f"Error reading Word file: {e}")
        else:  # It's an audio file object
            file_type = current_document.object_list[0].file_type
            file_path = current_document.object_list[0].input_path
            absolute_file_path = os.path.abspath(os.getcwd() + file_path)   

        # Set the audit and issue flags
        isaudit = bool(isaudit)
        isissue = bool(isissue)
